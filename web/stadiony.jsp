...
<%
                ...
                ResultSet rs = stat.executeQuery("select * from STADION;");
                      
                if(rs.next()){
                out.println("<g>" + "Lista obejmująca wszystkie obecne i byłe stadiony EKSTRAKLASY" + "</g>");
                %>
                <table rules="all" width="60%" cellpadding="3" align="center" bgcolor="#cdcdcd" bordercolor="#F3F3F3">
            <thead>
                <tr>
                    <th>ID Stadiony</th>
                    <th>Adres</th>
                ...
                out.println("Nie ma obecnie żadnych stadionów w bazie </br></br> "
                + "<img src='blad.png' width='300' />");}
                
               ResultSet rs1 = stat.executeQuery("select * from STADION;");
                while (rs1.next()) {
                    out.println("<tr>");
                    out.println("<td>" + rs.getString("ID_STADION") + "</td>");
                    out.println("<td>" + rs.getString("ADRES") + "</td>");
                    out.println("<td>" + rs.getString("MIEJSC_SDZ") + "</td>");
                    out.println("<td>" + rs.getString("MIEJSC_ST") + "</td>");
                    out.println("</tr>");
                }
                rs.close();
                rs1.close();
