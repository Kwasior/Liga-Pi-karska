...
<div id="zawartosc">
  <tbody>
            <%
                ...
                String msiedzace = request.getParameter("msiedzace");    
                ...
                ResultSet rs;
                rs = stat.executeQuery("select * from KLUBY join STADION on STADION.ID_STADION=KLUBY.ID_STADION where MIEJSC_SDZ>'" + msiedzace + "'");
                      
                if(rs.next()){
                out.println("<g>" + "Lista obejmująca wszystkie kluby posiadające stadiony mające pojemność większą niż " + msiedzace + "</g>");
                %>
                <table rules="all" width="65%" cellpadding="3" align="center" bgcolor="#cdcdcd" bordercolor="#F3F3F3">
            <thead>
                <tr>
                    <th>Nazwa Klubu</th>
                    <th>Miasto</th>
                ...
                out.println("Nie ma takiego stadionu/stadionów <a href='stadiony_ciekaw.jsp'><font color='black'>Spróbuj ponownie</font></a></br></br> "
                + "<img src='blad.png' width='300' />");}
                
               ResultSet rs1 = stat.executeQuery("select * from KLUBY join STADION on STADION.ID_STADION=KLUBY.ID_STADION where MIEJSC_SDZ>'" + msiedzace + "'");
                while (rs1.next()) {
                    out.println("<tr>");
                    out.println("<td>" + rs.getString("NAZWA_KLUBU") + "</td>");
                    out.println("<td>" + rs.getString("MIASTO") + "</td>");
                    out.println("<td>" + rs.getString("ADRES") + "</td>");
                    out.println("<td>" + rs.getString("MIEJSC_SDZ") + "</td>");
                    out.println("</tr>");
                }
                rs.close();
                rs1.close();
