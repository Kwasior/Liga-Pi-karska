...
<%
                ...
                ResultSet rs = stat.executeQuery("select * from KLUBY;");
                      
                if(rs.next()){
                out.println("<g>" + "Lista obejmująca wszystkie obecne i byłe zespoły EKSTRAKLASY" + "</g>");
                %>
                <table rules="all" width="80%" cellpadding="3" align="center" bgcolor="#cdcdcd" bordercolor="#F3F3F3">
            <thead>
                <tr>
                    <th>ID Klubu</th>
                    <th>Nazwa Klubu</th>
                ...
                out.println("Nie ma obecnie żadnych klubów w bazie </br></br> "
                + "<img src='blad.png' width='300' />");}
                
               ResultSet rs1 = stat.executeQuery("select * from KLUBY;");
                while (rs1.next()) {
                    out.println("<tr>");
                    out.println("<td>" + rs.getString("ID_KLUBU") + "</td>");
                    out.println("<td>" + rs.getString("NAZWA_KLUBU") + "</td>");
                    out.println("<td>" + rs.getString("STRONA_WWW") + "</td>");
                    out.println("<td>" + rs.getString("MIASTO") + "</td>");
                    out.println("<td>" + rs.getString("MISTRZ") + "</td>");
                    out.println("<td>" + rs.getString("PUCHAR") + "</td>");
                    out.println("<td>" + rs.getString("ID_STADION") + "</td>");
                    out.println("<td>" + rs.getString("ID_TRENERA") + "</td>");
                    out.println("</tr>");
                }
                rs.close();
                rs1.close();
